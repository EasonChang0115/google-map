<template>
  <div class="card">
    ...
    <div class="title">
      ...
      >{{ isOpen(data.properties.available) }}</div>
    </div>
    <div class="info">
      <div class="line">
      ...
      <div class="line">
        ...
        <div class="data">{{ data.properties.custom_note || data.properties.note }}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    maskClass(val) {
      ...
      else if (val < 1000) return "bad";
      else return "good";
    },
    isOpen(val) {
      if (val.length === 0) return "未知";
      const r = val.split("、");
